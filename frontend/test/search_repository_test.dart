...
import 'package:productivity_dashboard/features/search/search_models.dart';
import 'package:productivity_dashboard/repositories/search_repository.dart';
...
void main() {
  ...
  test(
      'searches offline with source and tag filters while preserving workspace isolation',
      () async {
    ...
    await repository.indexDocument(document(
        id: 'b-project',
        title: 'React Dashboard Other',
        workspace: 'workspace-b',
        type: 'project',
        tags: ['react']));

    final results = await repository.search('React',
        filters: const SearchFiltersModel(
  ...
  test('records search history and persists saved searches', () async {
    await repository.indexDocument(document(
        ...
        workspace: 'workspace-a',
        type: 'task'));
    await repository.search('Plan',
        filters: const SearchFiltersModel(workspaceId: 'workspace-a'));
    await repository.saveSearch(const SavedSearchModel(
        name: 'Sprint work', query: 'Plan', favorite: true));

    final history = await repository.loadHistory();
    final saved = await SearchRepository(await SharedPreferences.getInstance())
        .loadSavedSearches();
  ...
  test(
      'extracts deterministic study resources with concepts, definitions, formulas, dates, and questions',
      () async {
    final resource = await repository.study(
      sourceId: 'source-1',
      title: 'Operating Systems',
      text:
          'Process is a program in execution. Threads are lightweight execution units.\nT = 4\nReview in 2026.',
  ...
  test('builds smart collections and discovery from indexed local knowledge',
      () async {
    ...
    final collections = await repository.smartCollections(
        filters: const SearchFiltersModel(workspaceId: 'workspace-a'));
    final discovery = await repository.discovery(first);

    expect(collections.any((collection) => collection.itemIds.length >= 2),
        isTrue);
    expect(discovery.relatedResults.map((result) => result.documentId),
        contains('note-2'));
