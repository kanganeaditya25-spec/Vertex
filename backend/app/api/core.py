...
def capabilities() -> dict[str, Any]:
    return {
        "modules": ["ai", "analytics", "configuration", "event_bus", "search", "storage", "sync", "security", "notifications"],
        ...
        "privacy": {"telemetry_default": False, "search_index": "in_memory_runtime_index"},
    }


@router.get("/search")
def core_search(q: str = Query(min_length=1), limit: int = Query(default=20, ge=1, le=100)) -> list[dict[str, Any]]:
    return [asdict(hit) for hit in index.search(q, limit=limit)]
